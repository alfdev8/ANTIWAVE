...
pathLVAserver = pathAntiwave+"server installer\\server\\LVA\\"                            
...
import sys
sys.path.insert(1,pathResources)
...
def convertUnitsToFramesPositives(frameUnits,realFrames):
    ...
    elif realFrames[0] == 0 and realFrames[1] > 0: # 0
...
def convertUnitsToPixels(SubRegionX,renderer):    
    ...
    for x in unifyGroups:
        if x == pixelMinSize:
            print ("The render engine does not support this pixel meassures, reduce the # of processing cores to adjust "+str(pixelMinSize)+" pixels")
            break
...
def setRenderCoordinatesAE(rendermode,coordinates,renderType):
...
def loadSyntax(syntaxType,syntaxVersion,renderMode,renderType,frames,tiles=0):
    ...
    if syntaxType == "maya" and syntaxVersion =='10' and renderType==1 and renderMode == 1:
        ...
        a0=pathLVAserver+fileName.split(":\\")[1]
        b[fileNameIndex]='"'+str(a0)+'"' 
        ...
        b[renderFolderLVApath]='"'+a2+'renderToClient\\"'
...
##            
        ...
        a0=pathLVAserver+a.read().split(":\\")[1]
        b[fileName]='"'+a0+'"' 
        a.close()        
        ...
        b[renderFolderLVApath]='"'+pathLVAserver+d+'\\RENDER"' 
...
##        print (b[verboseMode])
        ...
        c=0
        while c< len (a):
            swap+= " -s " + str(frames[c][0])+" -e "+str(frames[c][1])# + " -v " + b[verboseMode]
            swap2.append(swap)
            swap=''
            c+=1
...
##        print (swap2)
        ...
        c=0
        while c< len(a):
            swap3.append(str(swap1[c])+swap2[c]+ " -rt "+str(cores[c]) + " -mem "+str(RAM[c])+" "+swapA)
            c+=1
...
##        print(b[LVApath])
        ...
        B0=pathLVAserver+fileName.split(":\\")[1]
        b[fileNameIndex]='' 
        b.remove("[file]")
...
##        print (swap9)
        ...
        c=0
        while c< len (a):
            swap+=" -s "+ str(frames[c][0])+" -e "+str(frames[c][1]) +" -mm "+RAM[c] + " -n "+str(cores[c]) +" "+swapA
            swap2.append(swap)
            swap=''
            c+=1
...
##            print (x)
    ...
    elif syntaxType == "maya" and syntaxVersion == '10' and renderType == 3 and renderMode==2 :
        print("Loading")
        



        print ("After Effects frame mode ")

        a=open(pathResources + "ae11Syntax.txt",'r')
        b=a.read().split("***ae")[1]
        ...
        b.pop(0)
        print(b)
        c=getEnviromentVariable.getAEpath()  
        ...
        b[renderFolderLVApath]='"'+pathLVAserver+'\\RENDER\\render*######@.png"'  
...
##        print (len(frames))
        ...
        c=0
        while c< len (a):
            swap+=" -s "+ str(frames[c][0])+" -e "+str(frames[c][1]) +" -mem_usage 100 80"
            swap2.append(swap)
            swap=''
            c+=1
        ...
        c=0
        while c< len(a):
            swap3.append(os.path.normpath(str(swap1[c])+swap2[c]))
