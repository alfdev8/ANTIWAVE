...
def sendProjectFolder():
    ...
    if len(ipAdress) == 1:
        ...
        c=0
        while c < len(ipAdress):
            subprocess.call([TempPath+"clienttoserver"+str(c)+".bat",'ls'],shell=False)
...
def sendScripts(renderMode,tiles=0):
    ...
    if renderMode == 2:
        ...
        while c < len(ip):
            ...
            a.write("python "+NetworkPath+"client.py "+ip[c]+" aestartrenderframes")
            a.close()
            c+=1
        ...
        while c < len(ip):
            ...
            a.write("python "+NetworkPath+"client.py "+ip[c]+" aestartmonitorframes")
        ...
        a.close()
        subprocess.call([pathLVA+'clientFrames.bat'],shell=False) 
...
def startMonitorFrames():
    ...
    while c < len(ip):
        ...
        subprocess.call([TempPath+"startmonitorframes"+'_'+str(c)+".bat"],shell=False)
        c+=1
...
def startMonitor():
    ...
    subprocess.call([TempPath+"startMonitorServerToClient.bat",'ls'],shell=False)
