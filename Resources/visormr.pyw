...
##        print (renderDirectories)            
        ...
        for x in cores:            
            ...
            while c<int(x):
                a=open(TempPath+"tileScripts\\"+hostnames[f]+'\\'+hostnames[f]+'_'+str(c)+'.bat','a') #estos statements de acceso a disco no van aqui con atributo a versiones
...
##            c+=1
        ...
        #networkSetup.extractRenderFramesFolder()       BUY WINRAR
...
buttonUploadProject = tk.Button(frame_buttons,text="Upload project folder", command=getFolderBrowserNetwork)     #automatic get last path, versions remember on close
...
checkRenderTypemr=tk.Checkbutton(frame_buttons,text="mentalray",variable=renderType,onvalue=1,offvalue=0,command=lambda:changeRenderEngine(True))
checkRenderTypeSw=tk.Checkbutton(frame_buttons,text="Maya Software",variable=renderType,onvalue=2, offvalue=0,command=lambda:changeRenderEngine(False))
checkRenderTypeAE=tk.Checkbutton(frame_buttons,text="After Effects",variable=renderType,onvalue=3, offvalue=0,command=lambda:changeRenderEngine(None))
checkRenderTypemr.select()
